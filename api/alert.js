...
const SEEN_BLOB_NAME = 'seen.json';
const KEYWORDS = ['소방', '소방설비', '소방시설', '소방점검', '소방안전관리자'];
...
async function loadSeen() {
  try {
    const { blobs } = await list({ prefix: SEEN_BLOB_NAME });
    const found = blobs.find((b) => b.pathname === SEEN_BLOB_NAME);
    if (!found) return {};
...
async function saveSeen(seen) {
  await put(SEEN_BLOB_NAME, JSON.stringify(seen, null, 2), {
    access: 'private',
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const [saramin, jobkorea, wanted] = await Promise.all([
      fetchSaramin(KEYWORDS).catch((e) => {
        console.error('사람인 오류:', e.message);
        return [];
      }),
      fetchJobKorea(KEYWORDS).catch((e) => {
        console.error('잡코리아 오류:', e.message);
        return [];
      }),
      fetchWanted(KEYWORDS).catch((e) => {
    ...
    if (fresh.length === 0) {
      res.status(200).json({ ok: true, message: '새로운 공고가 없어 메시지를 보내지 않음.' });
      return;
    }
    ...
    res.status(200).json({ ok: true, found: deduped.length, sent: fresh.length });
  } catch (e) {
